...
class AIWiringTests(unittest.TestCase):
    def test_aitool_is_read_off_the_handler(self):
        source = MATTERBOT.read_text()
        self.assertIn("'aitool'", source,
                      "the command registry must carry an 'aitool' key")
        self.assertIn("'aitool', False", source,
                      "the AI opt-in must be read off process() (@cmdutils.aitool) "
                      "and default to False, so an undecorated module is never "
                      "offered to the model")
        self.assertNotIn(
            "settings.AITOOL", source,
            "the AI opt-in must NOT be readable from defaults.py/settings.py: a "
            "config file could then widen exposure past what a developer marked "
            "safe, which is the inverse of how the gate is documented",
        )
