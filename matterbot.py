...
class MattermostManager(object):
    ...
    async def send_message(self, chanid, text, postid=None):
        try:
            channame = self.channelid_to_chaninfo(chanid)['name']
        ...
        if options.Matterbot['recursion'] or userid != self.my_id:
            ...
            for mline in messagelines:
                ...
                for idx,word in enumerate(message):
                    if (word in self.binds or word in options.Matterbot['helpcmds'] or word in options.Matterbot['mapcmds']) and not \
                       (message[idx-1] in options.Matterbot['helpcmds'] or message[idx-1] in options.Matterbot['mapcmds']):
