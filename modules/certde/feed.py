...
try:
    from modules.certde import defaults as settings
except ModuleNotFoundError: # local test run
    import defaults as settings
    if Path('settings.py').is_file():
        import settings
else:
    if Path('modules/certde/settings.py').is_file():
        try:
            from modules.certde import settings
        except ModuleNotFoundError: # local test run
            import settings
    ...
    for URL in settings.URLS:
        ...
        while count < MAX:
            try:
                ...
                filtered = False
                if "wid" in URL and "kritisch" in title: # Check for critical advisories
                    filtered = True
                elif "bsi" in URL:
                    filtered = True
                if filtered:
